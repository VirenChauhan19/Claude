...
- 60-second countdown with urgency stinger at 10 s, per-tick click sounds 9 → 1
...
**What I did instead:** I manually changed the confirmation wording to `"AGENT LOCKED"` (for the animated banner) and `"AGENT LOCKED"` in the sub-header, and directed AI to update `handleTimeout()` to say `"TIME EXPIRED! RANDOM AGENT ASSIGNED: [NAME]"`. This is documented in commit `431f333 fixed the lock agent text`.
...
| `431f333` | Fixed the lock agent text | Rejection Moment 3 — changed "Hero Locked" → "AGENT LOCKED" to match military theme |
| `a38ada3` | Update index.html | Final hero image swap from Unsplash → Pexels for better portrait compositions |
